 ...
 *
 */
export const ActionBar: FC<ActionBarProps> = ({ actions = [], themeKey = 'actionbar' }) => {
  const items = useMemo(() => {
    ...
    return sortedItems.map(({ node, onClick, 'aria-label': ariaLabel, group, href }, index) => {
      ...
      return (
        ...
        >
          {typeof node === 'string' ? (
            href ? (
              <Link variant={`${themeKey}.link`} href={href} aria-label={ariaLabel}>
                {node}
              </Link>
            ) : (
              <Button variant={`${themeKey}.button`} onClick={onClick} aria-label={ariaLabel}>
                {node}
              </Button>
            )
          ) : (
