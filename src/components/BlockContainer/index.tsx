...
const BlockContainer: FC<BlockContainerProps> = ({
  ...
  const content = !plain ? <Box variant="blockcontainer.inner">{children}</Box> : children
  return (
    <Box variant="blockcontainer.container" {...rest}>
      {(blockId || title || collapsible) && (
        <LinkHeading as={collapsible ? 'h3' : 'h4'} id={blockId} title={title} sx={{ my: 0, py: 1 }}>
          {title &&
            (collapsible ? (
              <Flex variant="blockcontainer.titleblock">
                ...
                >
                  <Text variant="blockcontainer.expandicon">
                    {isOpen ? <ChevronDownIcon verticalAlign="middle" /> : <ChevronRightIcon verticalAlign="middle" />}
                  </Text>
                </Link>
              </Flex>
            ) : (
      ...
      {<Box>{description}</Box>}
      {collapsible ? <Collapsible isOpen={isOpen}>{content}</Collapsible> : content}
      {!isOpen && <Divider />}
    </Box>
  )
}
